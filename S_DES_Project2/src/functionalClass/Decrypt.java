...
// 解密函数的书写
// 任务1：输入可以是8bit的秘文数据和10bit的密钥，输出是8bit的明文。
// 答：调用decryptData函数，输入是一个int类型的数组和10bits的秘钥key，进行解密，
// 输出是lengths长度为8的数组，数组的元素每一位为0,1
// 注：数组的每一个元素都是0,1，在这里我们假定数组的每一个元素代表一个bit
// 任务3：考虑到向实用性扩展，解密算法的数据输入可以是ASII编码字符串(分组为1 Byte)，
// 对应地输出也可以是ACII字符串(很可能是乱码)。
// 答：decryptASCII，输入是一个String类型的 ASCII编码字符串(也很可能是乱码)和10bits的秘钥key，
// 最后输出是一个String类型的解密 ASCII编码字符串
public class Decrypt {
    ...
    public static void main(String[] args) {
        ...
        String ciphertextASCII = "çlËcyË}\bÜé"; // 输入String类型的ASCII解密
    ...
    public static String decryptASCII(String ciphertextASCII, String key) {
        ...
        for (int i = 0; i < ciphertextASCII.length(); i++) {
            plaintextArray[i] = decryptData(ciphertextArray[i], key);
        }
    ...
    public static int[] decryptData(int[] ciphertext, String key) {
        ...
        // 第一层：初始置换
        int[] inputBlock = initialPermutation(IP, ciphertext);
        // 第二层：第一轮:f2，输入 k2 与初始置换的结果 IP(P)
        int[] result2_ni = roundFunction(k2, inputBlock);
        // 第三层：左右互换
        int[] swap_result = swapLeftAndRight(result2_ni);
        // 第四层：第二轮:f1，输入 k1 与左右互换的结果 swap_result
        int[] result1_ni = roundFunction(k1, swap_result);
        // 第五层：最终置换
        int[] final_result = initialPermutation(IP_ni, result1_ni);

        return final_result;
    }

    // 第一层：初始置换函数
    private static int[] initialPermutation(int[] IP, int[] ciphertext) {
        int[] permutedText = new int[IP.length];
        for (int i = 0; i < IP.length; i++) {
            permutedText[i] = ciphertext[IP[i] - 1];
        }
        return permutedText;
    }
